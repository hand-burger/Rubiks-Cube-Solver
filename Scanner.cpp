...
void scanCube()
{
	...
	for (size_t i = 0; i < faceScans.size(); i++)
	{
		...
		// Orange mask
		inRange(faceScans[i], Scalar(0, 59, 83), Scalar(19, 255, 255), faceMasks[1]);
		...
		// Green mask
		inRange(faceScans[i], Scalar(60, 45, 50), Scalar(76, 255, 158), faceMasks[4]);
	...
	// Now the cube is scanned and the cube array is filled (Hopefully)
